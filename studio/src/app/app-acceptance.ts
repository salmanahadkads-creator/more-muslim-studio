...
export const appProductReadiness: ToolcraftProductReadiness = {
  mode: "product",
  productName: "More Muslim Social Studio",
  productSummary:
    "Creates on-brand More Muslim Instagram posts (cover, quote, synopsis, now-streaming, and credits layouts) with approved colourways, star-lattice patterns, episode illustrations, and PNG export.",
  requestedBehavior:
    "Rebuild the legacy single-file social studio control panel on the Toolcraft stack: schema-driven controls for template, colourway, scene background, and per-template text, a live slide preview on the canvas, and real-resolution PNG/JPG export.",
};

const textControlRow = (
  id: string,
  target: string,
  fixture: string,
  observable: string,
  componentType: "code" | "text" = "text",
): ToolcraftComponentAcceptance => ({
  automated: true,
  automatedTestName: `schema: ${target} is a visible ${componentType} control with a default`,
  browser: true,
  browserTestName: `app controls: editing ${target} updates the slide text`,
  componentType,
  ...
  kind: "control",
  target,
  userAction: "Type into the field while its template is active.",
});

export const appAcceptance: readonly ToolcraftComponentAcceptance[] = [
  {
    automated: true,
    automatedTestName: "schema: post.template covers all five brand layouts",
    browser: true,
    browserTestName: "app controls: template select swaps the slide layout",
    componentType: "select",
    evidence: "product-output",
    expectedObservable:
      "Selecting each template renders that layout's structure on the canvas slide.",
  ...
  {
    automated: true,
    automatedTestName: "schema: post.colourway covers all nine approved pairings",
    browser: true,
    browserTestName: "app controls: colourway select restyles the slide ground and ink",
    componentType: "imagePicker",
    evidence: "product-output",
    expectedObservable:
      "The colourway picker shows a ground-colour swatch per pairing; choosing one repaints the slide ground, ink, pattern tile, and symbol per the approved pairing.",
    fixture: "Default cover post with pattern scene.",
    id: "post.colourway.picker",
    kind: "control",
    optionCoverage: "each-visible-item",
    target: "post.colourway",
    userAction: "Pick each colourway swatch in the Colourway picker.",
  },
  {
    automated: true,
    automatedTestName: "schema: scene.source gates illustration and upload controls",
    browser: true,
    browserTestName: "app controls: scene source switches pattern, solid, and image grounds",
    componentType: "select",
    evidence: "product-output",
    expectedObservable:
      "Pattern shows the star-lattice tile, solid shows the plain ground, illustration and upload show a full-bleed image; dependent controls appear only for their branch.",
    fixture: "Cover post cycling all four scene sources.",
    id: "scene.source.select",
    kind: "control",
    optionCoverage: ["pattern", "solid", "illustration", "upload"],
    target: "scene.source",
    userAction: "Choose each scene source option.",
  },
  {
    automated: true,
    automatedTestName: "schema: scene.illustration lists all ten episode artworks",
    browser: true,
    browserTestName: "app controls: episode picker swaps the slide illustration",
    componentType: "imagePicker",
    evidence: "product-output",
    expectedObservable:
      "Each picked episode renders its illustration full-bleed behind the slide text; the picker is visible only while scene.source is illustration and hidden for other sources.",
    fixture: "Cover post with scene source set to illustration.",
    id: "scene.illustration.picker",
    kind: "control",
    optionCoverage: "each-visible-item",
    target: "scene.illustration",
    userAction: "Pick each episode thumbnail in the Scene section.",
  },
  {
    automated: true,
    automatedTestName: "runtime: dragging the image repositions its crop",
    browser: true,
    browserTestName: "runtime: dragging the preview image moves the crop on both axes",
    componentType: "runtime",
    evidence: "product-output",
    expectedObservable:
      "When the scene ground is an illustration or upload, an on-canvas surface over the preview lets the user drag directly on the image to live-move the cover-crop focus on both axes (scene.imagePosition); it appears only for image sources.",
    fixture: "Cover post with an episode illustration active.",
    id: "runtime.sceneImage.crop",
    kind: "runtime",
    userAction: "Drag on the preview image horizontally and vertically.",
  },
  {
    automated: true,
    automatedTestName: "runtime: the under-image zoom slider scales the image",
    browser: true,
    browserTestName: "runtime: the under-image zoom slider scales the preview image",
    componentType: "runtime",
    evidence: "product-output",
    expectedObservable:
      "A zoom slider beneath the preview scales the image ground live while dragged (scene.imageZoom, 1-2x); it appears only for illustration or upload sources.",
    fixture: "Cover post with an episode illustration active.",
    id: "runtime.sceneImage.zoom",
    kind: "runtime",
    userAction: "Drag the under-image zoom slider from 1 toward 2.",
  },
  {
    automated: true,
    automatedTestName: "schema: scene.upload is a single-image fileDrop",
    browser: true,
    browserTestName: "app controls: uploading an image renders it as the slide ground",
    componentType: "fileDrop",
    evidence: "media-lifecycle",
    expectedObservable:
      "An uploaded image covers the slide at the current canvas size while the canvas size stays unchanged; rotate and flip actions update runtime media transform metadata and the rendered slide consumes the transform; removing it returns to the empty upload state; Reset clears it; the uploader is visible only while scene.source is upload and hidden for other sources.",
    fixture: "Scene source set to upload with a non-4:5 test image.",
    id: "scene.upload.fileDrop",
    kind: "control",
    target: "scene.upload",
  ...
  textControlRow(
    ...
    "content.episode",
    "Any template active.",
    "The shared episode marker re-renders on the active layout with the typed text.",
  ),
  textControlRow(
    "content.cover.presents.text",
    "content.cover.presents",
    "Cover template active; hidden for other templates.",
    "The eyebrow label above the title re-renders with the typed text; the field is visible only while the cover template is selected.",
  ),
  textControlRow(
    "content.cover.title.text",
    "content.cover.title",
    "Cover template active; hidden for other templates.",
    "The large uppercase title re-renders with the typed text; the field is visible only while the cover template is selected.",
  ),
  textControlRow(
    "content.quote.dialogue.code",
    "content.quote.dialogue",
    "Quote template active; hidden for other templates.",
    "Each Speaker: quote line renders as a speaker/quote pair; the field is visible only while the quote template is selected.",
    "code",
  ),
  textControlRow(
    "content.synopsis.body.code",
    "content.synopsis.body",
    "Synopsis template active; hidden for other templates.",
    "Blank-line-separated paragraphs render as stacked body copy with auto size; the field is visible only while the synopsis template is selected.",
    "code",
  ),
  textControlRow(
    "content.streaming.lines.code",
    "content.streaming.lines",
    "Now-streaming template active; hidden for other templates.",
    "Each row renders as an outro line under the Now Streaming heading; the field is visible only while the streaming template is selected.",
    "code",
  ),
  {
    automated: true,
    automatedTestName: "schema: content.credits.list is a visible creditsEditor control with a default",
    browser: true,
    browserTestName: "runtime: credits editor adds, edits, and removes a credit row",
    builtInFitCheck: {
      checkedBuiltIns: ["code", "text", "collectionActions", "actions"],
      closestBuiltIn: "collectionActions",
      productObservable:
        "Each row's Name and Title render as one stacked credit pair on the slide and in the PNG export; a hyphenated name such as Abdel-Magied stays whole because no delimiter parsing runs on the runtime value.",
      whyInsufficient:
        "A credit is one two-field entity (name plus role) edited per item. CollectionActions grows and shrinks the runtime array but its itemControl renders exactly one built-in per item, so it cannot express the paired Name and Title inputs a single credit needs. Code is one delimiter-parsed textarea where a hyphen inside a real name is indistinguishable from the name/role separator and corrupts the parsed runtime output. Text is a single line and owns no item list. Actions fire commands but own no per-item editable state.",
    },
    componentType: "creditsEditor",
    customControlCoverage: "all-custom-control-behavior",
    evidence: "product-output",
    expectedObservable:
      "Every credit renders as its own row with separate Name and Title boxes; editing either box updates that credit on the slide, Add credit appends a row, and the row X removes it; visible only while the credits template is selected.",
    fixture: "Credits template active; hidden for other templates.",
    id: "content.credits.list.editor",
    kind: "control",
    target: "content.credits.list",
    userAction: "Edit a credit's Name or Title box, add a credit, then remove one.",
  },
  {
    automated: true,
    automatedTestName: "schema: export.includeBackground defaults to true",
    browser: true,
    browserTestName:
      "app controls: include toggle hides the slide ground and exports transparent pixels",
    componentType: "switch",
    evidence: "product-output",
    expectedObservable:
      "Turning Include off hides the live preview product background and makes exported PNG pixels transparent behind the slide text and images; video output would keep the product background.",
    fixture: "Cover post with pattern scene.",
    id: "export.includeBackground.switch",
    kind: "control",
  ...
  {
    automated: true,
    automatedTestName: "schema: appearance.background defaults to ivory beige",
    browser: true,
    browserTestName: "app controls: background colour fills the export backdrop",
    componentType: "color",
    ...
    timelineCoverage: "keyframes",
    userAction: "Pick a new hex in the Background row and export.",
  },
  {
    automated: true,
    automatedTestName: "schema: export.image.format offers png and jpg",
    browser: true,
    browserTestName: "app controls: image format changes the exported file type",
    componentType: "select",
    evidence: "exported-bytes",
    expectedObservable: "PNG exports image/png bytes; JPG exports image/jpeg bytes.",
    fixture: "Cover post exported in both formats.",
    id: "export.image.format.select",
    kind: "control",
    optionCoverage: ["png", "jpg"],
    target: "export.image.format",
    userAction: "Choose each format and export.",
  },
  {
    automated: true,
    automatedTestName: "schema: export.image.resolution offers current, 2k, 4k, and 8k",
    browser: true,
    browserTestName: "app controls: image resolution changes exported pixel dimensions",
    componentType: "select",
    evidence: "exported-bytes",
    expectedObservable:
      "2K/4K/8K produce 2048/4096/8192px long-edge images; current produces retina canvas size.",
    fixture: "Cover post exported at two resolutions.",
    id: "export.image.resolution.select",
    kind: "control",
    optionCoverage: ["current", "2k", "4k", "8k"],
  ...
  },
  {
    ...
    browser: true,
    browserTestName: "app controls: export png downloads the rendered slide",
  ...
  {
    automated: true,
    automatedTestName: "schema: carousel.episode lists all ten episode sets",
    browser: true,
    browserTestName: "app controls: episode set select changes the built carousel",
    componentType: "select",
    evidence: "product-output",
    expectedObservable:
      "The chosen episode drives which titles, marker, and illustration Build episode set writes into the new slide layers.",
    fixture: "Empty carousel before building an episode set.",
    id: "carousel.episode.select",
    kind: "control",
    optionCoverage: [
      "ep1",
      "ep2",
    ...
    userAction: "Choose an episode, then click Build episode set.",
  },
  {
    actionCoverage: ["carousel-build-episode-set"],
    automated: true,
    automatedTestName: "schema: carousel action builds the episode set",
    browser: true,
    browserTestName: "app controls: carousel actions create slide layers",
    componentType: "actions",
    evidence: "product-output",
    expectedObservable:
      "Build episode set creates the five-slide cover/synopsis/synopsis/credits/streaming set and selects its cover.",
    fixture: "Default cover post with no slide layers.",
    id: "carousel.actions",
    kind: "control",
    target: "carousel.slides",
    userAction: "Click Build episode set in the Carousel section.",
  },
  {
    automated: true,
    automatedTestName: "filmstrip: the add button appends a slide",
    browser: true,
    browserTestName: "runtime: filmstrip add button creates a slide",
    componentType: "runtime",
    evidence: "product-output",
    expectedObservable:
      "The filmstrip below the preview shows a thumbnail per slide plus a + tile; clicking + snapshots the current post and appends a new selected slide.",
    fixture: "Single cover post before adding a carousel slide.",
    id: "runtime.filmstrip.add",
    kind: "runtime",
    userAction: "Click the + tile in the carousel filmstrip.",
  },
  {
    automated: true,
    automatedTestName: "filmstrip: hidden while the audiogram template is selected",
    browser: true,
    browserTestName: "runtime: audiogram mode hides the carousel filmstrip",
    componentType: "runtime",
    evidence: "product-output",
    expectedObservable:
      "Selecting the Audiogram template removes the filmstrip overlay — including the empty add-slide tile that otherwise floats over the canvas — because the audiogram is a single timeline-driven video, not a slide deck; existing slide layers persist and the strip returns when a static template is selected again.",
    fixture: "Default cover post with no slide layers.",
    id: "runtime.filmstrip.audiogram-hidden",
    kind: "runtime",
    userAction: "Change Template to Audiogram in the Post section.",
  },
  {
    automated: true,
    automatedTestName: "filmstrip: selecting a thumbnail swaps the slide",
    browser: true,
    browserTestName: "runtime: selecting a filmstrip slide swaps the slide values",
    componentType: "runtime",
    evidence: "product-output",
    expectedObservable:
      "Clicking a filmstrip thumbnail saves the current slide snapshot and renders the selected slide's template, colourway, scene, and copy.",
    fixture: "Episode set built so five slides exist.",
    id: "runtime.filmstrip.selection",
    kind: "runtime",
    userAction: "Click a different thumbnail in the carousel filmstrip.",
  },
  {
    automated: true,
    automatedTestName: "filmstrip: drag reorder renumbers the carousel ZIP",
    browser: true,
    browserTestName: "runtime: reordering filmstrip slides renumbers the exported ZIP",
    componentType: "runtime",
    evidence: "exported-bytes",
    expectedObservable:
      "Dragging a filmstrip thumbnail to a new position changes which slide-NN.png each slide becomes in the exported ZIP.",
    fixture: "Episode set built before reordering.",
    id: "runtime.filmstrip.reorder",
    kind: "runtime",
    userAction: "Drag a filmstrip thumbnail to a new position, then export.",
  },
  {
    automated: true,
    automatedTestName: "filmstrip: delete removes a slide",
    browser: true,
    browserTestName: "runtime: filmstrip delete removes a slide",
    componentType: "runtime",
    evidence: "product-output",
    expectedObservable:
      "Each filmstrip thumbnail carries a delete (×) control that removes that slide layer and its snapshot, dropping the slide count; the last remaining slide hides its delete control.",
    fixture: "Episode set built so five slides exist.",
    id: "runtime.filmstrip.delete",
    kind: "runtime",
    userAction: "Click the × on a filmstrip thumbnail.",
  },
  {
    automated: true,
    automatedTestName: "filmstrip: duplicate copies a slide",
    browser: true,
    browserTestName: "runtime: filmstrip duplicate adds a copy",
    componentType: "runtime",
    evidence: "product-output",
    expectedObservable:
      "Each filmstrip thumbnail carries a duplicate (⧉) control that appends a copy of that slide's values as a new selected slide, raising the slide count.",
    fixture: "Episode set built so five slides exist.",
    id: "runtime.filmstrip.duplicate",
    kind: "runtime",
    userAction: "Click the ⧉ on a filmstrip thumbnail.",
  },
  {
  ...
  {
    automated: true,
    automatedTestName: "schema: audiogram.crossfade toggles the speaker crossfade",
    browser: true,
    browserTestName: "runtime: audiogram motion controls change the frame",
    componentType: "switch",
    evidence: "product-output",
    expectedObservable:
      "Turning Speaker crossfade off holds the host ground for the whole clip; on, the ground blends to the guest ground at speaker changes; visible only for the audiogram template.",
    fixture: "Audiogram with a two-speaker SRT fixture.",
    id: "audiogram.crossfade.switch",
    kind: "control",
    target: "audiogram.crossfade",
    userAction: "Toggle Speaker crossfade in the Motion section.",
  },
  {
    automated: true,
    automatedTestName: "schema: audiogram.breathing toggles envelope breathing and drift",
    browser: true,
    browserTestName: "runtime: audiogram motion controls change the frame",
    componentType: "switch",
    evidence: "product-output",
    expectedObservable:
      "Turning Breathing & drift off freezes the pattern-tile breathing and ground push-in; on, they animate with the audio envelope and clip length; visible only for the audiogram template.",
    fixture: "Audiogram with audio and captions.",
    id: "audiogram.breathing.switch",
    kind: "control",
    target: "audiogram.breathing",
    userAction: "Toggle Breathing & drift in the Motion section.",
  },
  {
    automated: true,
    automatedTestName: "schema: audiogram.wordAccent toggles the spoken-word accent",
    browser: true,
    browserTestName: "runtime: audiogram motion controls change the frame",
    componentType: "switch",
    evidence: "product-output",
    expectedObservable:
      "Turning Word accent off keeps every caption word in ink; on, the spoken word carries the colourway accent then settles to ink; visible only for the audiogram template.",
    fixture: "Audiogram with captions.",
    id: "audiogram.wordAccent.switch",
    kind: "control",
    target: "audiogram.wordAccent",
    userAction: "Toggle Word accent in the Motion section.",
  },
  {
    automated: true,
    automatedTestName: "schema: audiogram.highlight offers auto, off, and choose",
    browser: true,
    browserTestName: "runtime: audiogram motion controls change the frame",
    componentType: "select",
    evidence: "product-output",
    expectedObservable:
      "Auto renders the auto-scored line as a large italic pull-quote, Off disables the highlight, Choose uses the highlight line number; visible only for the audiogram template.",
    fixture: "Audiogram with a multi-block SRT fixture.",
    id: "audiogram.highlight.select",
    kind: "control",
    optionCoverage: ["auto", "off", "choose"],
    target: "audiogram.highlight",
    userAction: "Choose each highlight mode in the Motion section.",
  },
  {
    automated: true,
  ...
  {
    automated: true,
    automatedTestName: "schema: audiogram.motionIntensity scales the frame's ambient movement",
    browser: true,
    browserTestName: "runtime: audiogram automation keyframes evaluate while scrubbing",
    componentType: "slider",
    evidence: "product-output",
    expectedObservable:
      "Dragging the slider live rescales the ground pan, push-in, Ken Burns drift, and breathing depth per frame (0% holds a still frame); its keyframe diamond adds a Motion intensity timeline-keyframe-row and scrubbing between keyframes renders the evaluated in-between intensity; visible only for the audiogram template and hidden otherwise.",
    fixture: "Audiogram with a two-block SRT fixture and short audio.",
    id: "audiogram.motionIntensity.slider",
    kind: "control",
    target: "audiogram.motionIntensity",
    timelineCoverage: "keyframes",
    userAction: "Drag Motion intensity, add a keyframe with its diamond, then scrub.",
  },
  {
    automated: true,
    automatedTestName: "schema: audiogram.captionSize scales the caption text",
    browser: true,
    browserTestName: "runtime: audiogram automation keyframes evaluate while scrubbing",
    componentType: "slider",
    evidence: "product-output",
    expectedObservable:
      "Dragging the slider live rescales the active caption and pull-quote text per frame; its keyframe diamond adds a Caption size timeline-keyframe-row and scrubbing between keyframes renders the evaluated in-between size; visible only for the audiogram template and hidden otherwise.",
    fixture: "Audiogram with a two-block SRT fixture and short audio.",
    id: "audiogram.captionSize.slider",
    kind: "control",
    target: "audiogram.captionSize",
    timelineCoverage: "keyframes",
    userAction: "Drag Caption size, add a keyframe with its diamond, then scrub.",
  },
  {
    automated: true,
    automatedTestName: "schema: audiogram.outro sets the closing-card lines",
    browser: true,
    browserTestName: "runtime: audiogram outro text renders",
    componentType: "code",
    evidence: "product-output",
    expectedObservable:
      "Each row of the outro field renders as one staggered-fade line on the closing now-streaming card; visible only for the audiogram template.",
    fixture: "Audiogram with audio long enough to reach the outro.",
    id: "audiogram.outro.code",
    kind: "control",
    target: "audiogram.outro",
    userAction: "Edit the outro lines in the Outro section.",
  },
  {
    automated: true,
  ...
  {
    automated: true,
    automatedTestName: "schema: keyframed audiogram values evaluate through the timeline evaluator",
    browser: true,
    browserTestName: "runtime: audiogram automation keyframes evaluate while scrubbing",
    componentType: "runtime",
    evidence: "timeline-output",
    expectedObservable:
      "With the timeline panel expanded, each keyframe-capable control shows a diamond that adds a keyframe; each keyframed control gets its own labelled timeline-keyframe-row (the automation lanes); changing the control at another playhead time updates/adds a keyframe on that row; and the renderer consumes evaluateToolcraftTimelineValue(s) so scrubbing between keyframes renders the interpolated value in the product output and in exports.",
    fixture: "Audiogram with a two-block SRT fixture, short audio, and the timeline expanded.",
    id: "runtime.timeline.keyframes",
    kind: "runtime",
    timelineCoverage: "keyframes",
    userAction:
      "Expand the timeline, add keyframes from control diamonds at two playhead times, then scrub between them.",
  },
  {
    automated: true,
    automatedTestName: "timeline: selecting the audiogram opens the extended transport",
    browser: true,
    browserTestName: "runtime: selecting the audiogram reveals the extended timeline",
    componentType: "runtime",
    evidence: "product-output",
    expectedObservable:
      "Switching the template to Audiogram shows the timeline transport in its extended form (play, scrubber, duration, loop) without touching the Setup Timeline switch; switching back to a static template hides the transport again. The extend fires only on the template transition, so collapsing the strip while staying in audiogram mode is respected.",
    fixture: "Default cover post before any timeline interaction.",
    id: "runtime.timeline.audiogram-extended",
    kind: "runtime",
    userAction: "Change Template to Audiogram in the Post section.",
  },
  {
    ...
    userAction: "Change a control, reload the page.",
  },
];

export const starterControlSectionInventory: readonly ToolcraftControlSectionInventoryEntry[] = [
  {
    entity: "Post layout and copy",
    groupingReason:
      "The template selector and its gated copy fields form one dependency group: the selector chooses the layout, the shared episode marker appears on every layout, and only the active template's fields are visible.",
    targets: [
      "post.template",
      "content.episode",
      "content.cover.presents",
  ...
  {
    entity: "Brand colourway",
    groupingReason:
      "The colourway is one whole-slide brand entity: ground, ink, pattern tile, and symbol change together.",
    targets: ["post.colourway"],
    title: "Colourway",
  },
  {
  ...
  {
    entity: "Carousel slide set",
    groupingReason:
      "The episode selector feeds the Build episode set action, and both actions manage the slide layers that make up the carousel.",
    targets: ["carousel.episode", "carousel.slides"],
    title: "Carousel",
  },
  {
  ...
  {
    entity: "Audiogram motion toggles",
    groupingReason:
      "The motion switches and the highlight selector all tune the same per-frame audiogram animation system, so they belong in one Motion group.",
    targets: [
      "audiogram.crossfade",
      "audiogram.breathing",
  ...
  {
    entity: "Audiogram outro copy",
    groupingReason: "The outro lines are the single closing-card copy block of the audiogram.",
    targets: ["audiogram.outro"],
    title: "Outro",
  },
  {
  ...
  },
  {
    ...
    groupingReason: "Format and resolution tune the same PNG/JPG delivery pass.",
