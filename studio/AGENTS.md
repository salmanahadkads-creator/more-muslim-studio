...
- **Sanctioned runtime deltas — re-apply these after ANY regeneration of `src/toolcraft`.** This app carries deliberate runtime edits the schema cannot express. A regeneration reverts them SILENTLY, and the integrity check will NOT catch it (regeneration re-hashes the manifest to match the reverted code). The guard against this is `src/app/toolcraft-runtime-deltas.test.ts` — if it goes red after a regeneration, a delta was lost; re-apply it and re-hash the manifest. Current deltas:
  1. **No timeline duration ceiling.** The upper `maxTimelineDurationSeconds` clamp was removed from all three duration-normalising sites (`runtime/state/reducer.ts`, `runtime/schema/define-toolcraft.ts`, `runtime/react/timeline-panel.tsx`) so a clip is as long as its audio (a 3-hour cut must not truncate). Keep only the 1s floor and the `Number.isFinite` guard. There is no schema option for max duration, so this must live in the runtime. See the worklog Iterations 12/14.
...
- `pnpm test` includes Toolcraft source integrity and local docs checks. If a desired control style is missing, fix the schema or regenerate from the upstream template/runtime; do not patch copied `src/toolcraft` files for one app — the one sanctioned exception is the runtime-delta list above.
